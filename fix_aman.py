...
# Read the Aman+Elc sheet specifically
excel_file = "attendace and timetables/كشوفات الغياب الاسبوعي.xlsx"
df = pd.read_excel(excel_file, sheet_name='Aman+Elc', header=None)

print(f"Sheet shape: {df.shape}")
print("Looking for all names in Aman+Elc:")
...
for i in range(df.shape[0]):
    for j in range(df.shape[1]):
        ...
        if pd.notna(cell_value) and 'Names' in str(cell_value):
            ...
            # Remove existing Aman+Elc students
            all_students = [s for s in all_students if s['groupId'] != 'aman_elc']
            
            # Add the corrected Aman+Elc students
            ...
            print(f"✅ Updated students.json with {len(students)} Aman+Elc students")
            break
    if names_found:
        break
